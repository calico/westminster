...
def collect_scores(out_dir: str, num_jobs: int, h5f_name: str = "sad.h5"):
    ...
    for key in job0_h5_open.keys():
        if key in ["target_ids", "target_labels"]:
            ...
            final_h5_open.create_dataset(
                key, shape=(num_rows,), dtype=job0_h5_open[key].dtype
            )

        else:
            num_targets = job0_h5_open[key].shape[1]
            final_h5_open.create_dataset(
                key, shape=(num_rows, num_targets), dtype=job0_h5_open[key].dtype
            )
            ...
            for key in job_h5_open.keys():
                try:
                    if key in ["target_ids", "target_labels"]:
                        ...
                        # re-index SNPs
                        final_h5_open[key][vgi : vgi + job_rows] = (
                            job_h5_open[key][:] + vi
                        )
    ...
    for key in snp_stats:
        if key == "pos":
            final_h5_open.create_dataset(key, data=np.array(snp_stats[key]))
        else:
            final_h5_open.create_dataset(key, data=np.array(snp_stats[key], dtype="S"))
...
def split_scores(it_out_dir: str, posneg: str, vcf_dir: str, sed_stats):
    ...
    for tissue_vcf_file in glob.glob(vcf_glob):
        ...
        with h5py.File("%s/sed.h5" % tissue_dir, "w") as tissue_h5:
            ...
            # write SNP i
            tissue_h5.create_dataset("si", data=np.array(sed_snpi))

            # write gene
            tissue_h5.create_dataset("gene", data=np.array(sed_gene, "S"))
