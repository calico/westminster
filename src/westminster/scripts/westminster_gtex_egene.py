...
def read_h5(h5_file, snp_stat, targets_file=None):
    with h5py.File(h5_file, "r") as borzoi_h5:
        ...
        scores_df["SNP_ID"] = list(
            borzoi_h5["si"]
        )  # Each si is a SNP ID, a single si can have multiple target genes
        ...
        for (
            col
        ) in scores_df.columns:  # Remove all RNA columns as we only need the mean score
            if "RNA" in col:
                del scores_df[col]
        ...
        variant_id_map.reset_index(
            inplace=True
        )  # Get index (borzoi['si'] for these variants, equivalent to a reset_index op)
...
def get_variant_scores(h5_file, snp_stat, genes_dict, targets_file=None):
    bz_scores_df, variant_id_map = read_h5(
        h5_file=h5_file, snp_stat=snp_stat, targets_file=targets_file
    )

    df = variant_id_map.merge(
        bz_scores_df, on="SNP_ID"
    )  # Each row now has SNP, SNP_ID, Gene and Bz (RNA) scores.
    bz_gene_type = [genes_dict[x.split(".")[0]]["gene_type"] for x in df["target_gene"]]
    df = df[
        np.array(bz_gene_type) == "protein_coding"
    ]  # Only keep scores for protein coding genes
    return df[["SNP_ID", "variant", "target_gene", "mean_score"]]


def read_gtf(gtf_file):
    genes_dict = gtf.parse_gtf_file(gtf_file)
    return genes_dict
...
def read_eqtls(f_name, genes_dict):
    ...
    print(
        "Number of cred. sets linked to protein-coding gene:",
        finemapped_prot_coding["molecular_trait_id"].nunique(),
    )
    finemapped_prot_coding = finemapped_prot_coding[
        ["chromosome", "position", "variant", "cs_id", "pip"]
    ]
...
def remove_indels(finemapped_dat, threshold=0.1):
    ...
    var_max_length = np.array(
        [max(len(x), len(y)) for x, y in zip(ref_allele, alt_allele)]
    )
    impacted_cred_sets = finemapped_dat["cs_id"][
        (var_max_length > 1) & (finemapped_dat["pip"] > threshold)
    ]
    remove_cred_set = np.array(
        [x in impacted_cred_sets.to_list() for x in finemapped_dat["cs_id"]]
    )
    ...
    print(
        "Number of cred. sets removed due to INDELs:",
        len(set(finemapped_dat["cs_id"])) - len(set(finemapped_dat_filt["cs_id"])),
    )
...
class Eval:
        ...
        for gene in genes_dict.keys():
            ...
            for transcript in genes_dict[gene]["transcripts"]:
                if transcript["transcript_type"] == "protein_coding":
                    genes_dict[gene]["tss"].append(
                        transcript["start"]
                    )  # strand has already accounted for in start
                else:
                    pass
        return genes_dict
    ...
    def cs_gene_scores(self, bz_scores, genes_dict, k):
        ...
        for idx in range(len(cred_sets)):
            cs = bz_scores[
                bz_scores["cs_id"] == cred_sets[idx]
            ]  # cred_sets[idx] is the egene name + an index.
            # For example: ENSG00000156876_L1
            egene = cred_sets[idx].split("_")[0]
            dat = pd.pivot_table(
                cs.astype({"mean_score": np.float64}),
            ...
            for col_name in dat.columns:
                if "ENSG" in col_name:  # If the column is a gene
                    ...
                    if tss_dict[gene_id][
                        "tss"
                    ]:  # If there is at least one protein-coding gene in the set
                        # compute borzoi score
                        pip_weighted_borzoi_score = sum(
                            dat["pip"] * np.abs(dat[col_name])
                        )
                        ...
                        for tss in tss_dict[gene_id]["tss"]:
                            ...
                            weighted_dist_score = sum(
                                dat["pip"] * (1 / np.abs(dat["DIST." + gene_id]))
                            )
                            tss_scores.append(weighted_dist_score)
                        dist_scores.append(
                            max(tss_scores)
                        )  # max. distance score across all alt. TSSs
                        genes.append(gene_id)
                    else:
                        # i.e. no protein coding transcripts exist for this gene
                        # exclude gene from analysis
                        print(
                            "No protein coding transcripts exist for gene: {}".format(
                                col_name
                            )
                        )
            ...
            if len(dat) == 0:
                print(
                    egene, "has no variants for which genes were scored in credible set"
                )
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-b", "--borzoi_sed_h5", help="Hdf5 file with Borzoi SED scores"
    )
    parser.add_argument("-e", "--eqtl", help="eqtl file path")
    parser.add_argument("-g", "--gtf", help="path to gencode gtf")
    parser.add_argument(
        "-k", "--topk", default=1, help="top k genes to consider for evaluation"
    )
    parser.add_argument(
        "-s",
        "--snp_stat",
    ...
    parser.add_argument("-t", "--targets_file", help="targets file path", default=None)
    ...
    bz_df = get_variant_scores(
        h5_file=args.borzoi_sed_h5,
    ...
    acc_bz, acc_dist = compute_linkage.cs_gene_scores(
        bz_scores=eqtl_bz_df, genes_dict=genes_dict, k=int(args.topk)
    )
