...
def main():
    parser = ArgumentParser(
        description="Compute variant effect predictions for paQTL SNPs in a VCF file using cross-fold model ensemble.",
    ...
    snp_group.add_argument(
        ...
        help="GTF for gene definition (required for gene-centered paQTL scoring)",
    )
    snp_group.add_argument(
        "--head",
        dest="head",
    ...
    snp_group.add_argument(
        "--stats",
        dest="snp_stats",
        default="covgene/logFC,covgene/JSD",
    ...
    gtex_group = parser.add_argument_group("GTEx paQTL options")
    ...
    gtex_group.add_argument(
        "--gtex",
        dest="gtex_vcf_dir",
        default="/home/drk/seqnn/data/gtex_v11/snp/paqtl",
        ...
        for ci in range(args.crosses):
            for fi in fold_index:
                ...
                for gtex_pos_vcf in glob.glob(f"{args.gtex_vcf_dir}/*_pos.vcf"):
                    ...
                    sad_pos = f"{it_out_dir}/{tissue_label}_pos/scores.h5"
                    sad_neg = f"{it_out_dir}/{tissue_label}_neg/scores.h5"
                    for snp_stat in snp_stats_gene:
                        stat_label = snp_stat.replace("/", "-")
                        class_out_dir = (
                            f"{it_out_dir}/{tissue_label}_class-{stat_label}"
                        )
...
def split_scores(it_out_dir: str, posneg: str, vcf_dir: str, snp_stats):
    ...
    # gene/covgene are written only by the stats that need them
    targets_opt = ["targets_gene.txt", "targets_covgene.txt"]
    ...
    with h5py.File(merge_h5_file, "r") as merge_h5:
        ...
        for tissue_vcf in glob.glob(f"{vcf_dir}/*_{posneg}.vcf"):
            ...
            tissue_label = tissue_label.replace("GTEx_txrev_", "")
            ...
            for tname in targets_opt:
                if os.path.exists(f"{merge_dir}/{tname}"):
                    shutil.copyfile(f"{merge_dir}/{tname}", f"{tissue_dir}/{tname}")
