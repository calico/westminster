...
def main():
    ...
    parser.add_option(
        "--fa",
        dest="genome_fasta",
        default="/home/yuanh/programs/genomes/hg38/hg38.fa",
    ...
    parser.add_option(
        "--weight_file",
        dest="weight_file",
        default="model0_best.h5",
        type="str",
        help="File name of the weight file.",
    )
    parser.add_option(
        "--shifts",
        dest="shifts",
    ...
    parser.add_option(
        ...
        default=1024,
        type=int,
        help="buffer batch size to call gradient on [Default: %default]",
    )
    parser.add_option(
        "--gene_file",
        dest="gene_file",
    ...
    seqnn_model.restore(model_folder + "/f0c0/train/%s"%options.weight_file, 0)
    ...
    print("GPU memory usage: %.2fGB" %mem)
    ...
    for fold_ix in options.folds:
        ...
        seqnn_model = seqnn.SeqNN(params_model)
        seqnn_model.restore(
            model_folder + "/f%dc0/train/%s"%(fold_ix, options.weight_file), 0)
        seqnn_model.build_slice(targets_df.index, False)
