...
def main():
  ...
  parser.add_option('-f', dest='num_folds',
    default=8, type='int',
  ...
  if X.shape[1] == 1:
    ...
    # else:
    aurocs, fpr_folds, tpr_folds, fpr_mean, tpr_mean, preds = randfor_roc(X, y, 
          folds=options.num_folds, iterations=options.iterations,
          min_samples_leaf=options.msl, random_state=options.random_seed)
...
def fold_roc(X: np.array, y: np.array,
  folds: int = 8,
  random_state: int = 44):
  """
...
def randfor_full(X: np.array, y: np.array,
  ...
  model = RandomForestClassifier(n_estimators=n_estimators, max_features='log2',
                                 max_depth=64, min_samples_leaf=min_samples_leaf,
                                 random_state=random_state, n_jobs=-1)
  model.fit(X, y)
  return model


def randfor_roc(X: np.array, y: np.array,
  folds: int = 8,
  iterations: int = 1,
  n_estimators: int = 100,
  min_samples_leaf: int = 1,
...
def ridge_roc(X: np.array, y: np.array,
  folds: int = 8,
  iterations: int = 1,
  alpha: float = 1,
  random_state: int = 44):
